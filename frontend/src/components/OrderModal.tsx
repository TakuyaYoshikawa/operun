        ...
        <div className="space-y-1 mb-3">
          {order.operations.map((op, i) => (
            <div key={op.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${editOpId === op.id ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50'}`}>
              ...
              {/* アクション */}
              <button onClick={() => lockMut.mutate(op.id)} disabled={lockMut.isPending} title={op.is_locked ? 'ロック解除' : 'ロック'} className="text-sm flex-shrink-0 opacity-60 hover:opacity-100">
                {op.is_locked ? '🔒' : '🔓'}
              </button>
